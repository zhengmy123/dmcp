...
func startServiceSync(ctx context.Context, store repository.ServiceStore, mgr *service.HTTPServiceManager) {
	cfg := config.LoadHTTPServiceStoreConfig()
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-ticker.C:
			syncServices(ctx, store, mgr)
		}
	}
}

func syncServices(ctx context.Context, store repository.ServiceStore, mgr *service.HTTPServiceManager) error {
	...
	for _, s := range services {
		storeMap[s.ID] = true
		if err := mgr.RegisterService(s); err != nil {
			log.Printf("register service %d failed: %v", s.ID, err)
		}
	}

	for _, s := range mgr.ListServices() {
		if !storeMap[s.ID] {
			if err := mgr.DeleteService(s.ID); err != nil {
				log.Printf("delete service %d failed: %v", s.ID, err)
			}
		}
	}
